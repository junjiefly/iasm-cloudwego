...
func compile() {
	...
	case _F_bin:
		err = os.WriteFile(fout, asm.Code(), 0755)
	case _F_elf:
		err = obj.ELF.Generate(fout, asm.Code(), uint64(asm.Base()), uint64(asm.Entry()))
	case _F_macho:
		err = obj.MachO.Generate(fout, asm.Code(), uint64(asm.Base()), uint64(asm.Entry()))
	default:
		panic("invalid format: " + ffmt)
	}
